...
fun first(list: Array<String>) {
    var count = 0;
    for (word in list) {
        if (checkWordFirst(word)) count++
    }
    println(count)
}

fun checkWordFirst(word: String): Boolean {
    val arr = word.split(" ")
    val nums = arr[0].split("-")
    val from = nums[0].toInt()
    val to = nums[1].toInt()
    val character = arr[1][0]
    val word = arr[2]
...
fun second(list: Array<String>) {
    var count = 0;
    for (word in list) {
        if (checkWordSecond(word)) count++
    }
    println(count)
}

fun checkWordSecond(word: String): Boolean {
    val arr = word.split(" ")
    val nums = arr[0].split("-")
    val from = nums[0].toInt()
    val to = nums[1].toInt()
    ...
    val first = word[from - 1] == character
    val second = word[to - 1] == character
